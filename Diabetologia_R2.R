...
#####proportion of all cancer out of all-cause bootstrap
for (n in c(1:500)) {
  detach(dmdata)
  dmdata = read_dta(paste0("db", n, ".dta"))
  nrow(dmdata)
  attach(dmdata)
  ...
  for (i in c(7:8)) {
    ...
    p1     <-as.data.frame(predict.glm(dmc1, newdata = nd1, type = "response"))
    colnames(p1)<-c("rate")
    
    out_overall[[i]]    <-cbind(nd1, p1, out = i, gender = NA, ethnic = NA, imd = NA, smk_status = NA, bmi_grp = NA, boots = n )
    ...
    for (j in c(1:2)) {
      ...
      p1     <-as.data.frame(predict.glm(dmc1, newdata = nd2, type = "response"))
      colnames(p1)<-c("rate")
      out_sex1[[j]] <-cbind(nd2, p1, out = i, gender = j, ethnic = NA, imd = NA, smk_status = NA, bmi_grp = NA, boots = n )
    }
    ...
    for (j in c(1:2)) {
      ...
      p1     <-as.data.frame(predict.glm(dmc1, newdata = nd2, type = "response"))
      colnames(p1)<-c("rate")
      out_eth1[[j]]    <-cbind(nd2, p1, out = i, gender = NA, ethnic = j, imd = NA, smk_status = NA, bmi_grp = NA, boots = n)
    }
    ...
    for (j in c(1:5)) {
      ...
      p1     <-as.data.frame(predict.glm(dmc1, newdata = nd2, type = "response"))
      colnames(p1)<-c("rate")
      out_imd1[[j]]    <-cbind(nd2, p1,out=i, gender = NA, ethnic = NA, imd = j, smk_status = NA, bmi_grp = NA, boots = n)
    }
    ...
    for (j in c(1:3)) {
      ...
      p1     <-as.data.frame(predict.glm(dmc1, newdata = nd2, type = "response"))
      colnames(p1)<-c("rate")
      out_smk1[[j]]    <-cbind(nd2, p1,out=i, gender = NA, ethnic = NA, imd = NA, smk_status = j, bmi_grp = NA, boots = n)
    }
    ...
    for (j in c(2:5)) {
      ...
      p1     <-as.data.frame(predict.glm(dmc1, newdata = nd2, type = "response"))
      colnames(p1)<-c("rate")
      out_bmi1[[j]]    <-cbind(nd2, p1,out=i, gender = NA, ethnic = NA, imd = NA, smk_status = NA, bmi_grp = j, boots = n)
    }
  ...
  write.dta(as.data.frame(out_all), file = paste0("results/DM_res", n, ".dta"))
}
